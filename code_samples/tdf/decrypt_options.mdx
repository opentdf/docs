...
Override the algorithm used for the ephemeral session key when unwrapping the data encryption key from the KAS. The default is `rsa:2048`.
...
// Metadata is available on the stream returned by read()
const stream = await client.read({
    source: { type: 'buffer', location: encryptedBytes },
});
