...
Specify which Key Access Server (KAS) will hold the key grants for this TDF. The KAS URL must be reachable by anyone who needs to decrypt the file.
...
```java
Config.TDFConfig config = Config.newTDFConfig(
    Config.withDataAttributes("https://example.com/attr/classification/value/secret"),
...
Assertions are cryptographically bound or signed statements attached to the TDF. They can carry structured metadata (e.g., classification markings, handling instructions) and can optionally be verified on decrypt.
...
statement.format = "json+stanag5636";
statement.schema = "urn:nato:stanag:5636:A:1:elements:json";
statement.value  = "{\"classif\":\"SECRET\"}";
...
To sign assertions with a key so that they can be verified on decrypt, add a `signingKey` (Go/Java) or `signers` map (JavaScript) to the assertion configuration.

---

### Wrapping Key Algorithm

Override the algorithm used to wrap the data encryption key when sending it to the KAS. The default is `rsa:2048`. Use an EC algorithm for smaller key access objects and faster wrapping operations.
