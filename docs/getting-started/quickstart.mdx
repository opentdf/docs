...
This guide will walk you through the process of securing data using OpenTDF tools. We'll help you set up a sandbox [OpenTDF platform](/architecture) locally with **attribute based access controls (ABAC)** to demonstrate the basic concepts of OpenTDF.
...
- **Entitlements** (what each person has access to):
  - **Jen**: `clearance/executive` (which includes `clearance/standard` via hierarchy)
