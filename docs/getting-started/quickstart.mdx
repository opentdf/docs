...
For this tutorial, we'll simulate our three personas by creating condition sets that match the default client credentials. Download the <a target="_blank" href="/quickstart/subject_condition_set.json" download="subject_condition_set.json">example</a> subject condition set as a template.

In a real deployment, you'd match identities by email, group membership, or other properties from your identity provider (like Keycloak). For now, we'll use the default client ID.
