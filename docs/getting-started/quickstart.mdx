...
If you didn't already create a profile during installation, do that now:
...
Then authenticate with your test credentials:
...
`--tls-no-verify` saves the setting to the profile, so all subsequent `otdfctl` commands — `encrypt`, `decrypt`, etc. — automatically skip certificate verification without needing the flag again. See [TLS Certificate Verification](/getting-started/managing-platform#tls-certificate-verification) for how to permanently trust the certificate instead.
:::
...
The `--tls-no-verify` flag used in Step 3 saves the setting to your profile, which is fine for CLI quickstart testing. However, if you're using browsers or SDKs, you may see certificate warnings.
...
**On Linux?** If you see cryptic errors like `"server misbehaving"` or silent failures, file permission issues are a common cause — see [File Permission Errors](/getting-started/managing-platform#file-permission-errors-linux).
