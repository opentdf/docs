...
```shell
# Skip TLS certificate verification for local development
...
`OTDFCTL_TLS_NO_VERIFY=true` applies to all subsequent `otdfctl` commands in your session. For persistent use, add it to your shell profile (`~/.zshrc` or `~/.bashrc`). See [TLS Certificate Verification](/getting-started/managing-platform#tls-certificate-verification) for how to permanently trust the certificate instead.
:::

:::note Linux Keyring Limitation
On Linux systems, you may see a warning: `Warning: Keyring storage is not available on Linux`. This is a known limitation. As a workaround, you can create the profile with credentials in one command (the `OTDFCTL_TLS_NO_VERIFY` env var set above still applies):

```shell
otdfctl profile create \
...
The `OTDFCTL_TLS_NO_VERIFY=true` environment variable set in Step 3 skips certificate validation for `otdfctl`, which is fine for CLI quickstart testing. However, if you're using browsers or SDKs, you may see certificate warnings.
