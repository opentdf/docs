...
1. **Use `--tls-no-verify` when creating your profile** (recommended):
   ```shell
   otdfctl profile create --tls-no-verify <profile-name> <url>
   ```
   This saves the setting to the profile, so all subsequent `otdfctl` commands using that profile automatically skip certificate verification — no flag needed on `encrypt`, `decrypt`, or other commands.
...
**Symptom:** Misleading errors like `"server misbehaving"`, `"connection reset"`, or generic gRPC failures — when the real cause is a file or directory permission problem. These appear because the process cannot read a system file needed for DNS resolution (`/etc/resolv.conf`, `/etc/hosts`), or cannot access the OpenTDF data directory.

| Scenario | Symptom | Fix |
|---|---|---|
| Docker volume ownership mismatch | Services start but fail silently on file ops | `sudo chown -R $USER:$USER ~/.opentdf/` |
| `/etc/hosts` or `/etc/resolv.conf` not readable | `"server misbehaving"` on all gRPC calls | `sudo chmod 644 /etc/resolv.conf /etc/hosts` |
| Install directory not writable | `mkdir` or download fails mid-install | Check permissions on `~/.opentdf/` |
...
docker run --rm -v ~/.opentdf/platform:/data alpine ls -la /data

# Check DNS config file permissions
ls -la /etc/resolv.conf /etc/hosts

# Fix ownership if needed
sudo chown -R $USER:$USER ~/.opentdf/
```
