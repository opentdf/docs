...
A Namespace is the parent container for a set of attributes. It can also optionally contain [registered resources](./registered_resources), [actions](./actions), [subject mappings](./subject_mappings), [subject condition sets](./subject_mappings#subject-condition-set), and [obligations](./obligations). Consider the following example:
...
- **Namespaces**:
  ...
  - Deletion (permanently removes the namespace and all associated attributes, values, registered resources, actions, subject mappings, and subject condition sets).
