...
A Namespace is the parent container for a set of attributes. It can also optionally contain [registered resources](./registered_resources), [actions](./actions), [subject mappings](./subject_mappings), subject condition sets, and [obligations](./obligations). Consider the following example:
