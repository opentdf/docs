...
func getDecision(client *sdk.SDK) {
    ...
    if resDecision.GetDecision() == authorization.Decision_DECISION_PERMIT {
        fmt.Println("Access granted")
        // Process any obligations
        if len(resDecision.GetObligations()) > 0 {
            fmt.Printf("Obligations to fulfill: %v\n", resDecision.GetObligations())
        }
    } else {
...
public void getDecision(SDK sdk) throws ExecutionException, InterruptedException {
    ...
    if (resp.getDecision().getDecision() == Decision.DECISION_PERMIT) {
        ...
        // Process any obligations
        if (resp.getDecision().getObligationsCount() > 0) {
            System.out.println("Obligations to fulfill: " + resp.getDecision().getObligationsList());
        }
    } else {
