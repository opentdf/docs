...
The Go v2 SDK provides convenience constructors that eliminate deeply nested proto construction:

| Helper | Description |
|--------|-------------|
| `authorizationv2.ForClientID(clientID)` | Identify by client ID |
| `authorizationv2.ForEmail(email)` | Identify by email address |
| `authorizationv2.ForUserName(username)` | Identify by username |
| `authorizationv2.ForToken(jwt)` | Resolve entity from JWT |
| `authorizationv2.WithRequestToken()` | Derive entity from the request's Authorization header |

These work with all v2 methods: `GetDecision`, `GetDecisionBulk`, and `GetEntitlements`.
