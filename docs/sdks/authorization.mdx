...
Every authorization call requires an [`Action`](/sdks/policy#action) — what's being requested (e.g. `decrypt`, `read`).
...
```java
Action.newBuilder().setName("decrypt").build();
```
