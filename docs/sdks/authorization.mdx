...
Identifies the data being accessed, by its attribute value FQNs (the same FQNs embedded in a TDF policy).

| Field | Type | Description |
|-------|------|-------------|
| `ephemeralId` | `string` | An ID you assign for correlating with the response. Used in bulk requests. |
| `attributeValues.fqns` | `[]string` | Attribute value FQNs on the resource (1–20). |
