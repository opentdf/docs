...
The core methods shown in current SDK docs and samples are:

| Operation | Go | Java | TypeScript |
| --- | --- | --- | --- |
| Encrypt to TDF | `CreateTDF` | `createTDF` | `createZTDF` |
| Load/read encrypted artifact | `LoadTDF` | `loadTDF` | `read` |
| Emit decrypted payload | `WriteTo` (reader) | `readPayload` (reader) | consume returned `DecoratedStream` |
