...
```javascript
import { authTokenInterceptor, clientCredentialsTokenProvider, attributeExists, attributeValueExists, OpenTDF } from '@opentdf/sdk';
...
if (!(await attributeValueExists(platformUrl, auth, marketingFqn))) {
  // Need the attribute ID to add a value — fetch it by FQN
