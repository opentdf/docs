...
The OpenTDF SDK uses ES module (`import`/`export`) syntax. Enable ES modules by adding `"type": "module"` to your `package.json`:
...
// Create the OpenTDF client — this generates DPoP keys but doesn't
// bind them to the auth provider until the first encrypt/decrypt call.
// We need to do that explicitly so PlatformClient can authenticate.
const client = new OpenTDF({ authProvider, platformUrl });
