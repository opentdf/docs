...
async function main() {
  ...
  // Token provider function for client credentials
  async function getAccessToken() {
    const resp = await fetch(`${oidcOrigin}/protocol/openid-connect/token`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: 'grant_type=client_credentials&client_id=opentdf&client_secret=secret',
    });
  ...
  const client = new OpenTDF({ interceptors: [authTokenInterceptor(getAccessToken)], platformUrl });
...
const interceptors = [authTokenInterceptor(getAccessToken)];
