...
The JavaScript SDK is designed for **browser applications**. For browser apps, start with [Refresh Token](#refresh-token) (the most common browser pattern). For backend scripts and testing, see [Client Credentials](#client-credentials). See the [Authentication Decision Guide](/guides/authentication-guide) for a full comparison.
:::
...
```typescript
import { authTokenDPoPInterceptor, clientCredentialsTokenProvider, OpenTDF } from '@opentdf/sdk';
