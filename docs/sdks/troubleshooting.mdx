...
2. **Use a built-in token provider**: The SDK's token providers handle caching and auto-refresh automatically:
...
**What changed**: Prior to v0.16.0, _all_ KAS 400-level errors were classified as `ErrTampered`. This made `errors.Is(err, sdk.ErrTampered)` unreliable — it returned `true` even for configuration errors. Starting in v0.16.0, the SDK distinguishes between tamper and misconfiguration:
...
- **JavaScript**: `await validateAttributes(platformUrl, auth, fqns)`
...
3. **Open a GitHub issue** on the relevant repo:
   - [opentdf/platform](https://github.com/opentdf/platform/issues) — Go SDK and platform
   - [opentdf/java-sdk](https://github.com/opentdf/java-sdk/issues) — Java SDK
   - [opentdf/web-sdk](https://github.com/opentdf/web-sdk/issues) — JavaScript/TypeScript SDK
