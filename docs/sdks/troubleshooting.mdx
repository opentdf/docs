...
2. **Implement token refresh in your application**: Use `authTokenInterceptor` with a function that returns a fresh token on each call. This ensures the SDK always has a valid token:
   ```js
   import { authTokenInterceptor, OpenTDF } from '@opentdf/sdk';

   // getAccessToken should fetch/refresh a token from your IdP
