...
| `/etc/resolv.conf` not readable | `"server misbehaving"` on any gRPC call | `sudo chmod 644 /etc/resolv.conf` |
| Docker volume ownership mismatch | Operations fail with cryptic errors | `sudo chown -R $USER ~/.opentdf/` |
