...
A list of `Attribute` objects, each with `fqn`, `rule`, and nested `values`. Paginates automatically — you always get the full list back without managing page tokens.
...
A list of attribute value FQN strings that the entity is entitled to access.
