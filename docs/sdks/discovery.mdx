...
<SdkVersion language="go" version="0.14.0" source="opentdf" />
...
<SdkVersion language="js" version="0.10.0" source="opentdf" />
...
const exists = await attributeExists(
  platformUrl,
  auth,
  "https://opentdf.io/attr/department",
);
if (!exists) {
...
const exists = await attributeValueExists(
  platformUrl,
  auth,
  "https://opentdf.io/attr/department/value/finance",
);
if (!exists) {
...
**JavaScript** uses `getEntitlements` instead (see [EntityIdentifier](/sdks/authorization#entityidentifier) for the request shape).
...
if (resp.entitlements.length > 0) {
  console.log(
    ...
    resp.entitlements[0].actionsPerAttributeValueFqn,
  );
}
```
