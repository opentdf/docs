...
- **Enumerated attributes** (with a fixed list of values): the value must match one of the registered values (case-insensitive).
- **Dynamic attributes** (no registered values): any string is accepted.

Use this instead of `ValidateAttributeExists` when you're working with dynamic attributes, or when you want to validate a value against an attribute's definition without constructing the full value FQN yourself.
...
:::tip Dynamic vs. enumerated attributes
If an attribute has no registered values (dynamic), `ValidateAttributeValue` always succeeds — use this when the platform administrator controls the allowed values out-of-band. If the attribute has registered values (enumerated), it acts as an allowlist and the call fails for any value not in the list.
:::
