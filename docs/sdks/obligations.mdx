...
const resp = await platform.v1.obligation.getObligationsByFQNs({
  fqns: ["https://example.com/obl/drm", "https://example.com/obl/audit"],
});
...
| `values`       | `[]string`      | No       | Initial values to create with the obligation. Can also be added later via [Create an Obligation Value](#create-an-obligation-value). |
...
console.log(
  `Created obligation: ${resp.obligation?.id}  FQN: ${resp.obligation?.fqn}`,
);
```
