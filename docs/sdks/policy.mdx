...
:::info Platform Service Client API
The methods on this page are **not** native to the core OpenTDF SDK. They are remote procedure calls made through a **platform service client** — a typed client that connects to your running OpenTDF platform over gRPC. All calls require a live platform endpoint and appropriate credentials.

This is the same pattern used by cloud provider SDKs (e.g., `new S3Client()` in AWS). You instantiate a client once, then call methods on it to manage resources on the platform.

**Required imports by language:**
...
import (
    ...
    // Plus the service-specific package for each method, e.g.:
...
```java
// Add the platform SDK to your dependencies, then:
...
See the [Auth Providers](/sdks/authorization) guide for credential setup.
:::

Policy in OpenTDF is the set of rules that govern who can access data and under what conditions. It is made up of **namespaces**, **attributes**, **subject mappings**, and **subject condition sets**.
