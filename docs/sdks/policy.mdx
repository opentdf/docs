...
`labels` is the only supported field. Other fields are silently ignored. When updating, use `metadataUpdateBehavior` to control whether labels are replaced or merged (see [Update a Namespace](#update-a-namespace) for an example).
...
```typescript
// Look up by UUID
let resp = await platform.v1.namespace.getNamespace({
...
console.log(
  `Attribute: ${resp.attribute?.name}, Rule: ${resp.attribute?.rule}`,
);
...
for (const [fqn, entry] of Object.entries(resp.fqnAttributeValues)) {
  console.log(
    `FQN: ${fqn} -> Attribute: ${entry.attribute?.name}, Value: ${entry.value?.value}`,
  );
}
```
...
console.log(
  `Associated subject mappings: ${resp.associatedSubjectMappings.length}`,
);
```
...
const resp =
  await platform.v1.subjectMapping.deleteAllUnmappedSubjectConditionSets({});
console.log(
  `Deleted ${resp.subjectConditionSets.length} unmapped subject condition sets.`,
);
```
...
A **subject mapping** links a subject condition set to a specific attribute value and a set of permitted actions (e.g., `DECRYPT`). It answers the question: _"which entities are allowed to do what with data carrying this attribute value?"_
