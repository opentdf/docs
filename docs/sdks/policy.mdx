...
A single `Namespace` object with `id`, `name`, `fqn`, `active` state, `metadata`, and timestamps.
...
| `metadata` | `MetadataMutable` | No | Labels and annotations. |
| `metadataUpdateBehavior` | `MetadataUpdateEnum` | No | `REPLACE` overwrites all labels; `EXTEND` merges with existing. |
